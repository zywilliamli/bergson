...
> **Double-blind review artifact.** Author names, affiliations, contact details,
> citation metadata, and the project's source-repository URL are intentionally
> withheld (`ANONYMIZED_REPOSITORY_URL`). Public Hugging Face model and dataset
> identifiers are retained unchanged so that every configuration in this
> artifact runs as-is. See [`REPRODUCING.md`](REPRODUCING.md) for the mapping
> from the paper's tables and figures to the commands that produce them.
...
|---|----------|-------------|
| [Notebook](notebooks/poison_detection.ipynb) | **Poison Detection** | Detect poisoned training examples with gradient attribution (T4, ~5 min) |
| [Notebook](notebooks/style_ablation.ipynb) | **Style Ablation** | Suppress style to recover semantic matching (A100, ~20 min) |
...
Citation metadata and author contact details are withheld during double-blind review.
